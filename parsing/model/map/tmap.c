...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/21 17:48:42 by user42            #+#    #+#             */
/*   Updated: 2021/02/21 17:50:59 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		check_map_char_valid(char **map)
{
	...
	while (map[i])
	{
		...
		while (map[i][e])
		{
			if (map[i][e] != '1' && map[i][e] != '2' && map[i][e] != '0' &&
			map[i][e] != 'N' && map[i][e] != 'S' && map[i][e] != 'W' &&
			map[i][e] != 'E' && map[i][e] != ' ' && map[i][e] != '\n')
				return (0);
			e++;
		}
		i++;
