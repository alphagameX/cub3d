...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/21 18:18:00 by user42            #+#    #+#             */
/*   Updated: 2021/02/21 20:16:25 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	resolution_check_after_init(t_game *game)
{
	int max_y;
	int max_x;

	mlx_get_screen_size(game->render.mlx, &max_x, &max_y);
	if (game->size.width > max_x)
		game->size.width = max_x;
	if (game->size.height > max_y)
		game->size.height = max_y;
}

void	start(t_game *game)
{
	resolution_check_after_init(game);
	set_dir(game);
	if (game->settings.bmp_required == 0)
		game->render.win = mlx_new_window(game->render.mlx, game->size.width,
		game->size.height, "cub3D");
	game->render.frame.img = mlx_new_image(game->render.mlx,
